...
function moveDir($s, $d)
{
    ...
    foreach ($test as $val) {
        if ($val != "." && $val != "..") {
            if (is_dir($s . "/" . $val)) {
                if (!is_dir($d . DS . $val)) mkdir($d . DS . $val, 0775, true);
                moveDir($s . "/" . $val, $d . "/" . $val);
            } else {
                $file_tot++;
                if (copy($s . "/" . $val, $d . "/" . $val)) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $zip = "https://github.com/MineWeb/MineWebCMS/archive/master.zip";
    $file = basename($zip);
    if (file_put_contents($file, file_get_contents($zip))) {
        $extract = new ZipArchive();
        if ($extract->open('master.zip') === true) {
            $extract->extractTo("./");
            $extract->close();
        }
    }
    moveDir(ROOT . DS . "install" . DS . 'MineWebCMS-master/', ROOT . DS);
    rrmdir(ROOT . DS . "install");
    return;
} else {
    include("verif-install.php");
}
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <link rel="stylesheet" href="./install/css/bootstrap.css" type="text/css" media="screen" title="no title"
          charset="utf-8">
    <link rel="stylesheet"
          href="https://fonts.googleapis.com/css?family=Roboto:400,300,500,700' rel='stylesheet' type='text/css">
    <link rel="stylesheet" href="./install/css/install/flat.css" type="text/css" media="screen" title="no title"
          charset="utf-8">
    <link rel="stylesheet" href="./install/css/install/animate.min.css" type="text/css">
    <link rel="stylesheet" href="./install/css/install/install.css" type="text/css" media="screen" title="no title"
          charset="utf-8">
    <link rel="stylesheet" href="./install/css/font-awesome.min.css" type="text/css" media="screen" title="no title"
          charset="utf-8">
...
<div class="container" style="max-width:700px">
    ...
    <form action="install.php" method="post">
        <button type="submit" class="btn btn-primary btn-block installSQL" style="display:none;">Télécharger MineWeb
        </button>
    </form>
...
<script src="./install/js/jquery.1.12.0.js"></script>
<script>
